...
: Node("kinect_ros2", options)
{
  ...
  rgb_info_.header.frame_id = "KapiBara_camera";
  depth_info_ = depth_info_manager_->getCameraInfo();
  depth_info_.header.frame_id = "KapiBara_camera";
...
void KinectRosComponent::timer_callback()
{
  ...
  header.frame_id = "KapiBara_camera_optical";
  ...
  if (_depth_flag) {
    ...
    cv::flip(_depth_image, img_fliped, 0);

    cv::Mat scaled;

    cv::resize(img_fliped,scaled,cv::Size(320,240),cv::INTER_AREA);

    auto msg = cv_bridge::CvImage(header, "16UC1", scaled).toImageMsg();
  ...
  if (_rgb_flag) {
    ...
    cv::flip(_rgb_image, img_fliped, 0);

    cv::Mat scaled;

    cv::resize(img_fliped,scaled,cv::Size(320,240),cv::INTER_AREA);

    auto msg = cv_bridge::CvImage(header, "rgb8", scaled).toImageMsg();
